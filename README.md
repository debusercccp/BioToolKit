...
│   └── hammDist.py        — distanza di Hamming tra due sequenze
│
├── search/            # ricerca pattern e motivi
│   ├── motifFind.py       — posizioni di una sottosequenza in una sequenza
│   ├── nGlicMotif.py      — motivo N-glicosilazione (N[^P][ST][^P]); fetch UniProt
│   ├── grepFastx.py       — grep regex su header di file FASTA/FASTQ
│   └── recSite.py         — siti di restrizione (palindromi inversi, k=4–12)
│
...
│   ├── seqMagique.py      — statistiche FASTA (min/max/avg len, num seqs) con Rich
│   └── blastOutput.py     — annota output BLAST -outfmt 6 con metadati da CSV
│
├── synthesis/         # generazione sequenze
│   └── synthSeq.py        — genera DNA sintetico via catena di Markov (k-mer training)
│
└── data/              # dati e risorse
    ├── fasta/             — sequenze scaricate (cache nGlicMotif)
    ├── out/               — output di default degli script
